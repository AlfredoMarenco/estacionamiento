            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                @switch($ticket->pagado)
                    ...
                    <div class="flex justify-center bg-orange p-3">
                        <h2 class="text-xl text-gray-100 font-bold">Orden: {{ $ticket->id }} | Estatus: No completado |
                            Creada el {{ $ticket->created_at }}
                        </h2>
                    </div>
                    <div class="grid grid-cols-4 gap-3 justify-items-center items-center">
                        ...
                        <div class="bg-gray-800 w-full text-center text-white font-bold rounded-t-md mt-4">Fecha de
                            Ingreso</div>
                        <div class="bg-gray-800 w-full text-center text-white font-bold rounded-t-md mt-4">Tiempo desde el
                            ingreso
                        </div>
                    ...
                    <div class="flex justify-center bg-gray-600 p-3">
                        <h2 class="text-xl text-gray-100 font-bold">Orden: {{ $ticket->id }} / Estatus: Cancelada fuera
                            de tiempo
                        </h2>
                    </div>
                    <div class="grid grid-cols-4 gap-3 justify-items-center items-center">
                    ...
                    <div class="grid grid-cols-4 gap-3 justify-items-center items-center">
                        ...
                        <div class="my-6">{{ $ticket->datetime_start }}</div>
                        <div class="my-6">{{ $ticket->datetime_end }}</div>
                        <div class=" flex col-span-4 my-2">
                            <form action="{{ route('printer.reprint', $ticket) }}" method="POST">
                                @method('POST')
                                @csrf
                                <button type="submit"
                                    class="mx-2 bg-gray-600 p-1 rounded-md text-center text-white font-semibold">Re-imprimir
                                    comprobante</button>
                            </form>
                            <form action="{{ route('ticket.cancel', $ticket) }}" method="POST">
                                @method('PUT')
                                @csrf
                                <button type="submit"
                                    class="mx-2 bg-red-600 p-1 rounded-md text-center text-white font-semibold">Cancelar
                                    Ticket</button>
                            </form>
                        </div>
                    </div>
